...
_viewJs.UI_ELEMENTS.BUTTON_NOW_SECTION.addEventListener("click", function() {
    ...
    _viewJs.makeBtnSectionBackgroundBlack("now");
});
_viewJs.UI_ELEMENTS.BUTTON_DETAILS_SECTION.addEventListener("click", function() {
    _viewJs.showInfoSection("details");
    _viewJs.makeBtnSectionBackgroundBlack("details");
});
_viewJs.UI_ELEMENTS.BUTTON_FORECAST_SECTION.addEventListener("click", function() {
    _viewJs.showInfoSection("forecast");
...
async function getWeather() {
    const formValue = _viewJs.UI_ELEMENTS.SEARCH_FIELD.value;
    ...
    _viewJs.UI_ELEMENTS.SEARCH_FORM.onsubmit = function() {
        return false;
    };
    try {
        ...
        function Weather() {
            this.name = json.name;
            this.temprature = _helpersJs.convertTemprature(json.main.temp);
            this.feelsLike = _helpersJs.convertTemprature(json.main.feels_like);
            this.weatherDescription = json.weather[0].main;
            this.sunriseTime = _helpersJs.convertTime(json.sys.sunrise);
            this.sunsetTime = _helpersJs.convertTime(json.sys.sunset);
...
async function getForecast(cityName) {
    ...
    _viewJs.showForecast(forecastArray);
}
function addFavoriteCity() {
    const cityName = _viewJs.UI_ELEMENTS.CITY_NAME_ELEMENTS[0].textContent;
...
function removeCity(cityName) {
    ...
    const ListItem = Array.from(citiesListItems).find((item)=>item.textContent === cityName
    );
    ListItem.remove();
    _viewJs.makeLikeImgWhite();
}
...
parcelHelpers.export(exports, "showWeather", ()=>showWeather
);
parcelHelpers.export(exports, "makeLikeImgBlack", ()=>makeLikeImgBlack
);
parcelHelpers.export(exports, "makeLikeImgWhite", ()=>makeLikeImgWhite
);
parcelHelpers.export(exports, "showInfoSection", ()=>showInfoSection
);
...
function showWeather(weatherObject) {
    ...
    UI_ELEMENTS.WEATHER_DESCRIPTION.textContent = weatherObject.weatherDescription;
    ...
    changeWeatherImage(weatherObject.weatherDescription, UI_ELEMENTS.WEATHER_IMAGE);
}
function showTempratureElements(temprature) {
    for (let number of UI_ELEMENTS.TEMPRATURE_NUMBERS)number.textContent = temprature;
}
const updateCityNameElements = (cityName)=>{
    ...
    for (let name of UI_ELEMENTS.CITY_NAME_ELEMENTS)name.textContent = cityNameUpdated;
};
...
function makeBtnSectionBackgroundBlack(sectionName) {
    switch(sectionName){
        case "now":
            UI_ELEMENTS.BUTTON_NOW_SECTION.classList.add("btn-black-background");
            UI_ELEMENTS.BUTTON_DETAILS_SECTION.classList.remove("btn-black-background");
            UI_ELEMENTS.BUTTON_FORECAST_SECTION.classList.remove("btn-black-background");
            break;
        case "details":
...
function showForecast(forecastArray) {
    ...
    forecastArray.forEach(function(item) {
        let { dt: date , main: { temp: temprature  } , main: { feels_like: feelsLike  } ,  } = item;
        const weatherDescription = item.weather[0].main;
        const convertedTemprature = _helpersJs.convertTemprature(temprature);
        const convertedFeelsLike = _helpersJs.convertTemprature(feelsLike);
        const convertedTime = _helpersJs.convertTime(date);
        const convertedDate = _helpersJs.convertDate(date);
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["huMbS","bDbGG"], "bDbGG", "parcelRequire634e")
